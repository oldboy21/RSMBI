...
def addUncPaths(targetDict, options):
    ...
    try:
        ...
        logger.error("Error while reading the unpaths file: " + str(e))

    for uncpath in uncpaths:
        server = uncpath.split('/')[2]
        path = '/'.join(uncpath.split('/')[3:])
        targetDict[server] = [path]
...
def unleashThreads(options, scope, db, targetDict, lock, targetIPs):
    ...
    for thread in range(options.T):
        try:
            worker = rsmbiworker("Worker-" + str(thread+1),
                                 options, targetDict, db, lock, scope, targetIPs)
            worker.start()
...
if __name__ == '__main__':
    ...
    logger.info("rSMBi has finished, cheers")
